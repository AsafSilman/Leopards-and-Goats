...
def main():
    ...
    """
    Controls drawing of board and start screen
    Places the Goats and the Leopards
...
def drawBoard(win): # DO NOT change this function. It is provided to help you.
    """
    It draws the board and returns a list of Points (24 points)
    """
    ...
    for i in range(1,4):
        ...
        pp = Polygon(ptList[-8:])
        pp.setWidth(5)
        pp.setOutline(color_rgb(255,0,0))
        pp.draw(win)
        time.sleep(0.09)
    for i in range(8):
        ll = Line(ptList[i],ptList[i+16])
        ll.setWidth(5)
        ll.setFill(color_rgb(255,0,0))
        ll.draw(win)
        time.sleep(0.09)

    #Extra code added to add circles at points of legal play to make UI more friendly for player"
    for i in ptList:
        cir = Circle(i,5)
        cir.setFill("yellow1")
...
def drawStart():
    """
    Draws the start menu layout and buttons
    Buttons and labels in order of [Play,Quit] in code.
    """
